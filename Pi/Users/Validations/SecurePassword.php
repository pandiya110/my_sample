...
class SecurePassword extends DataValidator {
    ...
    function doValidation($data) {
        ...
        if ($value == 1) {
            if (!empty($data['newPassword'])) {
                ...
                if (!empty($res)) {
                    if (!Hash::check($data['currentPassword'], $res->password)) {
                        $this->messages['password.is_dynamic_rule'] = 'Current Password is Invalid.Please Try Again';
                        return FALSE;
                    } else if (Hash::check($data['password'], $res->password)) {
