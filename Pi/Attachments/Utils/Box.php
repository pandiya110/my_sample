...
class Box {
    ...
    public function write_token($token, $type = 'file') {
        ...
        if (isset($array['error'])) {
            ...
            if ($type == 'file') {
                $fp = fopen($this->token_local_path.'token.box', 'w');
    ...
    public function read_token($type = 'file', $json = false) {
        if ($type == 'file' && file_exists($this->token_local_path.'token.box')) {
            $fp = fopen($this->token_local_path.'token.box', 'r');
            $content = fread($fp, filesize($this->token_local_path.'token.box'));
            fclose($fp);
        } else {
            return false;
        }
    ...
    function download($fileid) {
        $url = $this->build_url('/files/' . $fileid . '/content', array(), '');
        return $url;
        return json_decode($this->get($url), true);
    }

    function embededLink($fileid) {

        $url = $this->build_url('/files/' . $fileid, array('fields' => 'expiring_embed_link'), '');

        return json_decode($this->get($url), true);
    }

    function thumbnail($fileid, $ext) {
        $url = $this->build_url('/files/' . $fileid . '/thumbnail.' . $ext, array('min_height' => '450', 'min_width' => '450'), '');
        return json_decode($this->get($url), true);
    }
    ...
    function setTokenPath($path){
        $this->token_local_path=  $path;
    }
